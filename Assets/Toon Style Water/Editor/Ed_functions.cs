...
public static class edf
{
    ...
    public static List<int> search(List<string> from)
    {
        ...
        if (selectedSearchBar == searchBarInd)
        {
            ...
            edf.newLine();
            if ((lim < from.Count) && (searchInFocus)) edf.write("showing " + lim + " out of " + from.Count);
    ...
    public static bool foldedout = false;
    ...
    public static bool foldout(string txt, ref int selected, int current)
    {
        ...
        foldedout = (selected == current);

        if (EditorGUILayout.Foldout(foldedout, txt))
            selected = current;
        else
            if (foldedout) selected = -1;
    ...
    public static bool foldout(string txt)
    {
        ...
        foldedout = foldout(txt, ref selectedFold, foldInd);
    ...
    public static bool edit(ref Vector2 val)
    {
        checkLine();
        bool modified = false;
        modified |= edf.edit(ref val.x);
        modified |= edf.edit(ref val.y);
        return modified;
    }
